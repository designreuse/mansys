...
public class RoleController {
    ...
    public String create(RoleVO role, RedirectAttributes redirectAttributes) {
    ...
    public String showUpdateForm(@PathVariable("id") Long id, Model model) {
        setCommonData(model);
        RoleVO role = roleService.findOne(id);
        if(!CollectionUtils.isEmpty(role.getResourceList())) {
            StringBuilder sb = new StringBuilder();
            for (ResourceVO resourceVO : role.getResourceList()) {
                sb.append(resourceVO.getId());
                sb.append(",");
            }
            role.setResourceIds(sb.toString().substring(0, sb.toString().length() - 1));
        }
    ...
    public String update(RoleVO role, RedirectAttributes redirectAttributes) {
